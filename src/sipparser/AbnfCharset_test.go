...
func TestCharset(t *testing.T) {
	testdata := []struct {
		...
		{IsUriUnreserved, []byte("!'()*-.0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz~")},
		{IsUriReserved, []byte("@$&+,/:;=?")},
		{IsUriScheme, []byte("+-.0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz")},
		{IsUriUric, []byte("!$&'()*+,-./0123456789:;=?@ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz~")},
		{IsUriUricNoSlash, []byte("!$&'()*+,-.0123456789:;=?@ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz~")},
		{IsUriPchar, []byte("!$&'()*+,-.0123456789:=@ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz~")},
		{IsUriRegName, []byte("!$&'()*+,-.0123456789:;=@ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz~")},
		...
		{IsSipQuotedText, []byte("\x09\x0a\x0d !#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[]^_`abcdefghijklmnopqrstuvwxyz{|}~\xc0\xc1\xc2\xc3\xc4\xc5\xc6\xc7\xc8\xc9\xca\xcb\xcc\xcd\xce\xcf\xd0\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf\xe0\xe1\xe2\xe3\xe4\xe5\xe6\xe7\xe8\xe9\xea\xeb\xec\xed\xee\xef\xf0\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8\xf9\xfa\xfb\xfc\xfd")},
		...
		{IsSipPname, []byte("!$&'()*+-./0123456789:ABCDEFGHIJKLMNOPQRSTUVWXYZ[]_abcdefghijklmnopqrstuvwxyz~")},
		{IsSipPvalue, []byte("!$&'()*+-./0123456789:ABCDEFGHIJKLMNOPQRSTUVWXYZ[]_abcdefghijklmnopqrstuvwxyz~")},
		{IsSipHname, []byte("!$'()*+-./0123456789:?ABCDEFGHIJKLMNOPQRSTUVWXYZ[]_abcdefghijklmnopqrstuvwxyz~")},
		{IsSipHvalue, []byte("!$'()*+-./0123456789:?ABCDEFGHIJKLMNOPQRSTUVWXYZ[]_abcdefghijklmnopqrstuvwxyz~")},
		...
		{IsTelPhoneDigit, []byte("()-.0123456789")},
	...
	for i, v := range testdata {
		...
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			for _, ch := range v.chars {
				test.EXPECT_TRUE(t, v.funcPtr(ch), "ch = 0x%02x", ch)

			}

			for _, ch := range makeNotContains(v.chars) {
				test.EXPECT_FALSE(t, v.funcPtr(ch), "ch = 0x%02x", ch)
			}
		})
	}
